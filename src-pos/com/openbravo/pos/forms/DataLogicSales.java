...
public class DataLogicSales extends BeanFactoryDataSingle {
    ...
    public final ProductInfoExt getProductInfoByName(String sName) throws BasicException {
        return (ProductInfoExt) new PreparedSentence(s
            ...
            , SerializerWriteString.INSTANCE
            , ProductInfoExt.getSerializerRead()).find(sName.toUpperCase());
    }
    ...
    public final SentenceExec getProductEan13Update() {
        return new SentenceExecTransaction(s) {
            public int execInTransaction(Object params) throws BasicException {
                Object[] values = (Object[]) params;
                int i = new PreparedSentence(s
                    , "UPDATE PRODUCTS SET ID = ?, CODE = ? WHERE ID = ?"
                    , new SerializerWriteBasicExt(productsRow.getDatas(), new int[]{0, 1, 0})).exec(params);
                return i;
            }
        };
    }
